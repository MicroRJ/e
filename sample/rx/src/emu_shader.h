...
typedef struct
{
  ...
  /* rxShader expected_color_attachments :: for pixel shaders only,
...
/* beware that this function may not handle padding very well should
 ...
 try to pack things into float4's if nothing else works. #important */
	void
	rlShaderIni(rxShader *shader, rxShaderConfig *config);

	rxShader
	rxShaderCreateFromBytecode(int flags, char const *label, size_t length, void *memory);
